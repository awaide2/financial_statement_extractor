    ...
    for page_no in candidate_pages:
        ...
        for i, dpi in enumerate(dpi_list):
            ...
            if DEBUG_MODE:
                ...
                # Only show meaningful progress (not all DPI attempts)
            ...
            score = (
                3 if status == 'SUCCESS_CONSOLIDATED' else 2 if status == 'SUCCESS_STRUCTURE_ONLY' else 1,
                major_hits,
                col_conf,
                -diff if diff != float('inf') else -1e12,
                row_count
            )
